...
;; EXPRESIONES
    ...
    (expresion ("for" "("expresion "to" expresion ")" "{" expresion "}") for-exp) ;;PHYTON

    (expresion ("print" "("expresion")") print-exp) ;; C
        

;*******************************************************************************************
;;  DEFINICIONES EN AMBITOS PRIVADOS
    (expresion ("const" identificador "=" expresion )constante-exp) ;; C
    (expresion ("var" identificador "=" expresion )variable-exp) ;; C
...
;*******************************************************************************************
     ...
     (print-exp (exp) (begin
                         (display "console: ")
                         (display (eval-expresion exp env))
                         (display "\n")))
      (else #f))))
...
;; FOR AUX 
(define loop-aux
  (lambda (start stop body env)
    (letrec((index (- stop start))
            (return (lambda (n proc)
                   (if (> n 0)
                       (begin
                       (eval-expresion body env)
                         proc(return (- n 1) proc))
                       (eval-expresion body env)))))
      (return index body))))
...
for(1 to 5){print('hola')}

private def (var $x = 1,var $lista = []){
 module{
 for(1 to 5){
  module{
   print($x)
   now $lista = ($lista push  [$x] )
   now $x = ++($x)
   $lista}}
}}
...
private def (
 var $x = 1,
 var $lista = [],
 const $isPar = func ($n) => {if (($n mod 2) == 0){true}else{false} }){
 module{
 for(1 to 5){
  module{
   print(import $isPar ($x))
   now $lista = ($lista push  [import $isPar ($x)] )
   now $x = ++($x)
   $lista}}
}}
